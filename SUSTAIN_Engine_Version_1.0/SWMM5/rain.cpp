...
float      UnitsFactor;                // units conversion factor
...
{
    int   i, k;
    INT4  kount = count;               // number of gages in data file
    INT4  filePos1;                    // starting byte of gage's header data
    INT4  filePos2;                    // starting byte of gage's rain data
    INT4  filePos3;                    // starting byte of next gage's data
    INT4  interval;                    // recording interval (sec)
    INT4  dummy = -1;
    ...
    fwrite(&kount, sizeof(INT4), 1, Frain.file);
    ...
    for ( i = 0;  i < count; i++ )
    {
        ...
        for ( k = 1; k <= 3; k++ )
            fwrite(&dummy, sizeof(INT4), 1, Frain.file);
    }
    ...
    for ( i = 0; i < Nobjects[GAGE]; i++ )
    {
        ...
        if ( addGageToRainFile(i) )
        {
            ...
            interval = Interval;
            fwrite(staID,      sizeof(char), MAXMSG+1, Frain.file);
            fwrite(&interval,  sizeof(INT4), 1, Frain.file);
            fwrite(&filePos2,  sizeof(INT4), 1, Frain.file);
            fwrite(&filePos3,  sizeof(INT4), 1, Frain.file);
            filePos1 = ftell(Frain.file);
            filePos2 = filePos3;
...
//
{
    ...
    fread(&kount, sizeof(INT4), 1, Frain.file);
...
//
{
    int   k;
    INT4  interval;
    INT4  filePos1, filePos2;
    char  staID[MAXMSG+1];

    for ( k = 1; k <= kount; k++ )
    {
        fread(staID,      sizeof(char), MAXMSG+1, Frain.file);
        fread(&interval,  sizeof(INT4), 1, Frain.file);
        fread(&filePos1,  sizeof(INT4), 1, Frain.file);
        fread(&filePos2,  sizeof(INT4), 1, Frain.file);
        if ( strcmp(staID, Gage[i].staID) == 0 )
        {
            // --- match found; save file parameters
            Gage[i].rainType     = RAINFALL_VOLUME;
            Gage[i].rainInterval = (int)interval;
...
//
{
    ...
    if ( !isMissing )
    {
        //date2 = date1 + datetime_encodeTime(hour, minute, 0) - TimeOffset;

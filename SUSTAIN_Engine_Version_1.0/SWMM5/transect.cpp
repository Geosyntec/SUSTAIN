...
static float  Station[MAXSTATION+1];   // x-coordinate of each station
static float  Elev[MAXSTATION+1];      // elevation of each station
static float  Nleft;                   // Manning's n for left overbank
static float  Nright;                  // Manning's n for right overbank
static float  Nchannel;                // Manning's n for main channel
static float  Xleftbank;               // station where left overbank ends
static float  Xrightbank;              // station where right overbank begins
static float  Xfactor;                 // multiplier for station spacing
static float  Yfactor;                 // factor added to station elevations
...
//-----------------------------------------------------------------------------
/* moved to func.h
static int    setParams(int transect, char* id, float x[]);
static int    setManning(float n[]);
static int    addStation(float x, float y);
static float  getFlow(int k, float a, float wp, int findFlow);
static void   getGeometry(int i, int j, float y);
static void   getSliceGeom(int k, float y, float yu, float yd, float *w,
                           float *a, float *wp);
*/
////////////////////////////////////////
//  New function defined. (LR - 3/10/06)
////////////////////////////////////////
void setMaxSectionFactor(int transect);
...
//
{
    ...
    switch ( k )
    {
      ...
      case 2:
        ...
        while ( i < ntoks )
        {
            if ( ! getFloat(tok[i], &x[1]) )
                return error_setInpError(ERR_NUMBER, tok[i]);
            if ( ! getFloat(tok[i+1], &x[2]) )
...
///////////////////////////////////////////
//  New function call added. (LR - 3/10/06)
///////////////////////////////////////////
...
int  setParams(int j, char* id, float x[])
//
//  Input:   j = transect index
//           id = transect ID name
...
int  addStation(float y, float x)
//
...
{
    int   k;                 // station index
    float ylo,               // lower elev. of transect slice
...
void getSliceGeom(int k, float y, float ylo, float yhi, float *w,
                  float *a, float *wp)
//

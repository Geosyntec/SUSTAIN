...
//added
static void splitTSS(int j, int sedflag, char* ID);
static void splitBUILDUP(int j, int p);
static void splitWASHOFF(int j, int p);
...
//
{
	...
	//added
    char  wLine2[MAXLINE+1];           // working copy2 of input line  
    ...
    for (i = 0; i < MAX_NODE_TYPES; i++) Nnodes[i] = 0;    ////Added (LR - 3/10/06)
    for (i = 0; i < MAX_LINK_TYPES; i++) Nlinks[i] = 0;    ////Added (LR - 3/10/06)
    ...
    while ( fgets(line, MAXLINE, Finp.file) != NULL )
    {
		...
		//added
        strcpy(wLine2, line);          // make working copy of line 
        Ntokens = getTokens(wLine2);
		...
		{
			//check if the pollutant is TSS to split it into sand, silt, and clay
			if (sect == s_POLLUTANT)
			{
				char  strSand[10];  
				char  strSilt[10];  
				...
				if ( Ntokens >= 7 )
				{
					nsedFlag = findmatch(Tok[6], SedimentWords);             
						if ( nsedFlag < 0 ) return error_setInpError(ERR_KEYWORD, Tok[6]);
				}

				if (nsedFlag == 1)
				{
					//the pollutant is sand
					errcode = addObject(sect, tok);
					nSAND++;
				}
				else if (nsedFlag == 2)	
				{
					//the pollutant is silt
					errcode = addObject(sect, tok);
					nSILT++;
				}
				else if (nsedFlag == 3)	
				{
					//the pollutant is clay
					errcode = addObject(sect, tok);
					nCLAY++;
				}
				else if (nsedFlag == 4)	
				{
					//split TSS into sand, silt, and clay
					sstrncpy(strTSS, Tok[0], MAXFNAME);
					//strTSS = Tok[0];
					nTSS++;
					errcode = addObject(sect, tok1);
					nSAND++;
					errcode = addObject(sect, tok2);
					nSILT++;
					errcode = addObject(sect, tok3);
					nCLAY++;
				}
				else
				{
					//this pollutant is not a sediment
					errcode = addObject(sect, tok);
				}
			}
			else
	...
	if (nTSS > 1 || nSAND > 1 || nSILT > 1 || nCLAY > 1)
		return error_setInpError(ERR_KEYWORD, Tok[6]);
...
//
{
    ...
    switch (sect)
    {
      ...
      case s_POLLUTANT:
		...
		//if pollutant is TSS then need to split into sand, silt, and clay
		nsedFlag = findmatch(Tok[6], SedimentWords); 
		if (nsedFlag == 4)
		{
			//SILT
      ...
      case s_BUILDUP:
        err = landuse_readBuildupParams(Tok, Ntokens);

		//if pollutant is TSS then need to split into sand, silt, and clay
		if(strncmp(Tok[1],strTSS,MAXFNAME) == 0) 
		{
			int j = project_findObject(LANDUSE, Tok[0]);
			if ( j < 0 ) return error_setInpError(ERR_NAME, Tok[0]);
			//SILT
			Tok[1] = "SILT";
			int p = project_findObject(POLLUT, Tok[1]);
      ...
      case s_WASHOFF:
        err = landuse_readWashoffParams(Tok, Ntokens);
      ...
      case s_LOADING:
        err = subcatch_readInitBuildup(Tok, Ntokens);

		//if pollutant is TSS then need to split into sand, silt, and clay
		for (k = 2; k <= Ntokens; k = k+2)
		{
			if(strncmp(Tok[k-1],strTSS,MAXFNAME) == 0) 
			{
				...
				//SILT
				Tok[k-1] = "SILT";
				int p = project_findObject(POLLUT, Tok[k-1]);
				if ( p < 0 ) return error_setInpError(ERR_NAME, Tok[k-1]);
				// --- store loading in subcatch's initBuildup property
				Subcatch[j].initBuildup[p] = Subcatch[j].initBuildup[p-1];

				//CLAY
				Tok[k-1] = "CLAY";
				p = project_findObject(POLLUT, Tok[k-1]);
				if ( p < 0 ) return error_setInpError(ERR_NAME, Tok[k-1]);
				// --- store loading in subcatch's initBuildup property
				Subcatch[j].initBuildup[p] = Subcatch[j].initBuildup[p-1];
			}
...
//=============================================================================
//added
void splitTSS(int j, int sedflag, char* ID)
{
	Pollut[j].ID         = ID;        
	Pollut[j].sedflag    = sedflag;   
	Pollut[j].coFraction = Pollut[j-1].coFraction;
	Pollut[j].coPollut   = Pollut[j-1].coPollut;  
	Pollut[j].gwConcen   = Pollut[j-1].gwConcen;  
	Pollut[j].kDecay     = Pollut[j-1].kDecay;    
	Pollut[j].mcf        = Pollut[j-1].mcf;       
	Pollut[j].pptConcen  = Pollut[j-1].pptConcen; 
	Pollut[j].rdiiConcen = Pollut[j-1].rdiiConcen;
	Pollut[j].snowOnly   = Pollut[j-1].snowOnly;  
	Pollut[j].units      = Pollut[j-1].units;     
}

void splitBUILDUP(int j, int p)
{
	Landuse[j].buildupFunc[p].coeff[0]   = Landuse[j].buildupFunc[p-1].coeff[0];        
	Landuse[j].buildupFunc[p].coeff[1]   = Landuse[j].buildupFunc[p-1].coeff[1];        
	Landuse[j].buildupFunc[p].coeff[2]   = Landuse[j].buildupFunc[p-1].coeff[2];        
	Landuse[j].buildupFunc[p].funcType   = Landuse[j].buildupFunc[p-1].funcType;        
	Landuse[j].buildupFunc[p].maxDays    = Landuse[j].buildupFunc[p-1].maxDays;       
	Landuse[j].buildupFunc[p].normalizer = Landuse[j].buildupFunc[p-1].normalizer;        
}

void splitWASHOFF(int j, int p)
{
	Landuse[j].washoffFunc[p].bmpEffic   = Landuse[j].washoffFunc[p-1].bmpEffic;        
	Landuse[j].washoffFunc[p].coeff      = Landuse[j].washoffFunc[p-1].coeff;        
	Landuse[j].washoffFunc[p].expon      = Landuse[j].washoffFunc[p-1].expon;        
	Landuse[j].washoffFunc[p].funcType   = Landuse[j].washoffFunc[p-1].funcType;        
	Landuse[j].washoffFunc[p].sweepEffic = Landuse[j].washoffFunc[p-1].sweepEffic;        
}

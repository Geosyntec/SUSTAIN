...
static float**  OldIfaceValues;        // interface flows & WQ at previous time
static float**  NewIfaceValues;        // interface flows & WQ at next time
static float    IfaceFrac;             // fraction of interface file time step
...
static int   isOutletNode(int node);             //New function (LR-5/10/06)
...
//
{
    ...
    {
        // --- check that node has no outflow links
        //if ( Node[i].degree != 0 ) continue;        //Deleted (LR-5/10/06)
        if ( !isOutletNode(i) ) continue;             //Added (LR- 5/10/06)
...
//
{
    ...
    for (i=0; i<Nobjects[NODE]; i++)
    {
        //if ( Node[i].degree == 0 ) n++;             //Deleted (LR-5/10/06)     
        if ( isOutletNode(i) ) n++;                   //Added (LR-5/10/06)
    }
    ...
    for (i=0; i<Nobjects[NODE]; i++)
    {
        //if ( Node[i].degree == 0 || Node[i].type == OUTFALL ) //Deleted (LR-5/10/06)
          if ( isOutletNode(i) )                                //Added (LR-5/10/06)
            fprintf(Foutflows.file, "\n%s", Node[i].ID);
    }
...
//////////////////////////////////////////
//  This is a new function. (LR - 5/10/06)
//////////////////////////////////////////

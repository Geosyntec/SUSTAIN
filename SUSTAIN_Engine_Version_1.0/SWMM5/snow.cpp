...
//  Local functions
//-----------------------------------------------------------------------------
static void  setMeltParams(int i, int k, float x[]);
static float getRainmelt(float rainfall);
static float getArealDepletion(TSnowpack* snowpack, int i, float snowfall,
             float tStep);
static float getArealSnowCover(int i, float awesi);
static float meltSnowpack(TSnowpack* snowpack, int i, float rmelt, float asc,
             float snowfall, float tStep);
static float reduceColdContent(TSnowpack* snowpack, int i, float smelt,
             float ccFactor);
static float routeSnowmelt(TSnowpack* snowpack, int i, float smelt, float asc,
             float rainfall, float tStep);
static void  updateColdContent(TSnowpack* snowpack, int i, float asc,
             float snowfall, float tStep);
...
//
{
    ...
    float f;                           // fraction of impervious area plowable
    float snowDepth = 0.0;             // snow depth on entire subcatchment (ft)
    ...
    f = Snowmelt[k].snn;
    snowpack->fArea[IMPERV0] = f * Subcatch[j].fracImperv;
    snowpack->fArea[IMPERV1] = (1.0 - f) * Subcatch[j].fracImperv;
    snowpack->fArea[PERV]    = 1.0 - Subcatch[j].fracImperv;
...
void snow_setMeltCoeffs(int j, float s)
//
//  Input:   j = snowmelt parameter set index
...
void snow_plowSnow(int j, float tStep)
//
//  Input:   j     = subcatchment index
//           tStep = time step (sec)
//  Output:  none
...
//
{
    ...
    float rainfall;                    // rainfall (not used)
    float snowfall;                    // snowfall (ft/sec)
    float exc;                         // excess snow depth (ft)
    float f;                           // area ratio
    float sfracTotal;                  // total fraction of snow moved
    TSnowpack* snowpack;               // ptr. to snow pack object
...
float snow_getSnowMelt(int j, float rainfall, float snowfall, float tStep,
                       float netPrecip[])
//
//  Input:   j = subcatchment index
...
{
    int    i;                          // snow sub-area index
    float  rmelt;                      // melt rate when rain falling (ft/sec)
    float  smelt;                      // snow melt from sub-area (ft/sec)
    float  asc;                        // frac. of sub-area snow covered
    float  snowDepth = 0.0;            // snow depth on entire subcatchment (ft)
    float  impervPrecip;               // net precip. on imperv. area (ft/sec)
...
{
    int    i;
    float  snowCover = 0.0;            // snow cover volume (ft3)
    TSnowpack* snowpack;               // ptr. to snowpack object
...
float getArealDepletion(TSnowpack* snowpack, int i, float snowfall, float tStep)
//
...
//
{
    ...
    float asc;                         // fraction of area with 100% cover
    float si;                          // snow depth at 100% cover
    float awesi;                       // depth relative to depth at 100% cover
    float awe;
    float sba;
    float sbws;
...
float getArealSnowCover(int i, float awesi)
//
...
//
{
    ...
    float  asc1, asc2;                 // asc values at ends of interval
...
float meltSnowpack(TSnowpack* snowpack, int i, float rmelt, float asc,
                  float snowfall, float tStep)
//
//  Input:   snowpack = ptr. to snowpack object
//           i        = snow sub-area index
...
{
    int   k;                           // snowmelt parameter set index
    float smelt;                       // melt rate over sub-area (ft/sec)
    float ccFactor;                    // cold content conversion factor
...
float getRainmelt(float rainfall)
//
...
//
{
    float uadj;                        // adjusted wind speed
    float t1, t2, t3;

////////////////////////////////////////////////////
//  New intermediate variable added. (LR - 10/19/05)
////////////////////////////////////////////////////
    float smelt;                       // snow melt in in/hr
...
//////////////////////////////////////////////////////
//  Snowmelt converted back to ft/sec. (LR - 10/19/05)
//////////////////////////////////////////////////////
...
void updateColdContent(TSnowpack* snowpack, int i, float asc, float snowfall,
                       float tStep)
//
//  Input:   snowpack = ptr. to snowpack object
//           i        = snow sub-area index
...
{
    int   k;                           // snowmelt parameter set index
    float ati;                         // antecdent temperature index (deg F)
    float cc;                          // snow pack cold content (ft)
    float ccMax;                       // max. possible cold content (ft)
...
float reduceColdContent(TSnowpack* snowpack, int i, float smelt, float ccFactor)
//
//  Input:   snowpack = ptr. to snowpack object
//           i        = snow sub-area index
...
//
{
    float cc;                          // cold content of snow pack (ft)
...
float routeSnowmelt(TSnowpack* snowpack, int i, float smelt, float asc,
                    float rainfall, float tStep)
//
//  Input:   snowpack = ptr. to snowpack object
//           i        = snow sub-area index
...
{
    int    k;                          // snowmelt parameter set index
    float  vmelt;                      // snow melt volume (ft)

...
//  Local functions
//-----------------------------------------------------------------------------
static float getPatternFactor(int p, int month, int day, int hour);
...
/////////////////////////////////////////////////////////////////////////
////  Entire function rewritten to include new attributes. (LR - 7/5/06 )
/////////////////////////////////////////////////////////////////////////
//
...
//
{
    ...
    float cf = 1.0;                    // units conversion factor
    float sf = 1.0;                    // scaling factor
    float baseline = 0.0;              // baseline value
...
//
////////////////////////////////////////////////////
//  Support for new attributes added. (LR - 7/5/06 )
////////////////////////////////////////////////////
{
    int   k = inflow->tSeries;       // time series index
    float cf = inflow->cFactor;      // units conversion factor
    float sf = inflow->sFactor;      // scaling factor
    float blv = inflow->baseline;    // baseline value
    float tsv = 0.0;                 // time series value
...
float inflow_getDwfInflow(TDwfInflow* inflow, int month, int day, int hour)
//
...
//    patternID  value(n)  value(n+1) ....          (for continuation lines)
{
    ...
    {
        i = Pattern[j].count;
        if ( !getFloat(tok[n], &Pattern[j].factor[i]) )
...
float getPatternFactor(int p, int month, int day, int hour)
//

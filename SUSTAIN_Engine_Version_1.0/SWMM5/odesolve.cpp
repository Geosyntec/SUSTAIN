...
int rkqs(float* x, int n, float htry, float eps, float* hdid,
         float* hnext, void (*derivs)(float, float*, float*));

// function that performs the Runge-Kutta integration step
void rkck(float x, int n, float h, void (*derivs)(float, float*, float*));
...
int odesolve_open(int n)
{
    ...
    yscal = (float *) calloc(n, sizeof(float));
    dydx  = (float *) calloc(n, sizeof(float));
    yerr  = (float *) calloc(n, sizeof(float));
    ytemp = (float *) calloc(n, sizeof(float));
    ak    = (float *) calloc(5*n, sizeof(float));

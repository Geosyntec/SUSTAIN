...
//////////////////////////////////////////
////  Completely rewritten. (LR - 7/5/06 )
//////////////////////////////////////////
...
//  Purpose: allocates memory for a matrix of floats.
//
{
    int i,j;
    float **a;
    ...
    a = (float **) malloc(nrows * sizeof(float *));
    if ( !a ) return NULL;
    
    // --- allocate rows and set pointers to them
    a[0] = (float *) malloc (nrows * ncols * sizeof(float));
    if ( !a[0] ) return NULL;
...
////////////////////////////////////////
////  Compltely rewritten (LR - 7/5/06 )
////////////////////////////////////////
void project_freeMatrix(float **a)
//
//  Input:   a = matrix of floats
//  Output:  none
//  Purpose: frees memory allocated for a matrix of floats.
//
{
    if ( a != NULL )
    {
...
//
{
    ...
    switch ( k )
    {
      ...
      //     use slope weighting with dynamic wave flow routing, to
      //     apply normal flow limitation to dynamic wave flow routing,
      //     or skip flow routing during steady state periods.
      //     (NO = 0, YES = 1)
      case ALLOW_PONDING:
...
////////////////////////////////////
      case IGNORE_RAINFALL:
        ...
        switch ( k )
        {
          ...
          case NORMAL_FLOW_LTD:   NormalFlowLtd   = m;  break;
...
//
{
   ...
   NormalFlowLtd   = FALSE;            // Normal flow limit based on slope
   ...
   Compatibility   = SWMM4;            // Use SWMM 4 dyn. wave weighting method
...
//
{
    ...
    for (j = 0; j < Nobjects[SUBCATCH]; j++)
    {
        ...
        Subcatch[j].oldQual = (float *) calloc(Nobjects[POLLUT], sizeof(float));
        Subcatch[j].newQual = (float *) calloc(Nobjects[POLLUT], sizeof(float));

///////////////////////////////////////////////////////////////////
//  New properties added to support washoff routing. (LR - 7/5/06 )
///////////////////////////////////////////////////////////////////
        Subcatch[j].pondedQual = (float *) calloc(Nobjects[POLLUT], sizeof(float));
        Subcatch[j].totalLoad  = (float *) calloc(Nobjects[POLLUT], sizeof(float));
    }
    for (j = 0; j < Nobjects[NODE]; j++)
    {
        Node[j].oldQual = (float *) calloc(Nobjects[POLLUT], sizeof(float));
        Node[j].newQual = (float *) calloc(Nobjects[POLLUT], sizeof(float));
    ...
    {
        Link[j].oldQual = (float *) calloc(Nobjects[POLLUT], sizeof(float));
        Link[j].newQual = (float *) calloc(Nobjects[POLLUT], sizeof(float));
    }
    ...
    for (j = 0; j < Nobjects[SUBCATCH]; j++)
    {
        ...
        for (k = 0; k < Nobjects[LANDUSE]; k++)
        {
            Subcatch[j].landFactor[k].buildup =
                (float *) calloc(Nobjects[POLLUT], sizeof(float));
            Subcatch[j].landFactor[k].detstorage =
                (float *) calloc(Nobjects[POLLUT], sizeof(float));
        }
    }
...
//
{
    ...
    if ( Subcatch ) for (j = 0; j < Nobjects[SUBCATCH]; j++)
    {
        for (k = 0; k < Nobjects[LANDUSE]; k++)
        {
            ...
            FREE(Subcatch[j].landFactor[k].detstorage);
        }

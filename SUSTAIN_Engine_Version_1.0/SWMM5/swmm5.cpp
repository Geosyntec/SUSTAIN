...
/*
#ifdef DLL
	//#pragma argsused	// it is required for Borland C compiler (commented out 02-2007)
...
//  Purpose: opens a SWMM project.
   ...
   _fpreset();              
...
#endif
    {
        ...
        if ( EndDate == NO_DATE ) EndDate = StartDate;
        if ( ReportStartDate == NO_DATE ) ReportStartDate = StartDate;
        if ( ReportStartTime == NO_DATE ) ReportStartTime = StartTime;
        ...
        if ( RptFlags.input ) report_writeInput();
    }
...
DLLEXPORT swmm_start(int saveResults)
{
...
//
...

    {
        ...
        if ( !IgnoreRainfall ) rain_open();      ////Modified (LR - 3/10/06)
        ...
        if ( Nobjects[LINK] > 0 ) DoRouting = TRUE;
...
//           returns error code
//  Purpose: advances the simulation by one reporting time step.
//
    ...
    {
        // --- if next reporting time has not exceeded total duration
        if ( ReportTime <= TotalDuration )
        {
            // --- route flow & WQ through drainage system until
            //     next reporting time is reached (runoff will be
            //     calculated as needed).
            while ( NewRoutingTime < ReportTime ) execRouting(*elapsedTime);

            // --- save results at next reporting time
            if ( SaveResultsFlag ) output_saveResults(ReportTime);

            // --- update elapsed time (days) & next reporting time (msec)
            *elapsedTime = ReportTime / MSECperDAY;
...
///////////////////////////////////////////////////////////////////////////
// Modified to always execute runoff, even when there are no subcatchments,
// so that climate variables can be updated over time. (LR - 9/5/05)
///////////////////////////////////////////////////////////////////////////
...
//
    ...
    if ( IsStartedFlag )
    {
        ...
        if ( !IgnoreRainfall ) rain_close();     ////Modified (LR - 3/10/06)
...
//
{
    ...
    hour = (elapsedTime / 1000.0 / 3600.0);
